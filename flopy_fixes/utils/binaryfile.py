...
class BinaryHeader(Header):
    ...
    @staticmethod
    def create(bintype=None, has_reclen=False, **kwargs):
        """
...
def binaryread_struct(file, vartype, shape=(1), charlen=16, skip_bytes=0): # TO 170702, skip_bytes added
    """
...
def binaryread(file, vartype, shape=(1), charlen=16, skip_bytes=0):
    """
...
class BinaryLayerFile(LayerFile):
    ...
    def __init__(self, filename, precision, verbose, kwargs):
        ...
        self.skip_bytes = get_reclen_bytes(filename) #  170702
    ...
    def _build_index(self):
        ...
        self.file.seek(0, 2) # move pointer to end of file
        ...
        while ipos < self.totalbytes:
            ...
            self.file.seek(self.skip_bytes, 1)  # TO 170702
    ...
    def _get_header(self):
        ...
        #Here we could verify the file to see if the header contains record
        #length bytes.
        #f = self.file.
        #f.seek(0, -1)
        #b1 = f.read(1)
        #b2 = f.read(b1)
        #b3 = f.read(1)
        #if b1==b2, then we have a header with bytes, so add that info:
        #    self.record_length=b1
        #else:
        # self.record_length=None
        header = binaryread(self.file, self.header_dtype, (1,),
                            skip_bytes=self.skip_bytes) # TO 170702 skip_bytes added
        return header[0] # header[0] because an array is yielded
    ...
    def get_ts(self, idx):
        ...
        for k, i, j in kijlist:
            #recordlist = []  # TO recordlist was never used
...
class CellBudgetFile(object):
    ...
    def __init__(self, filename, precision='single', verbose=False, **kwargs):
        ...
        self.skip_bytes = get_reclen_bytes(filename) # TO 170702
    ...
    def _build_index(self):
        ...
        while ipos < self.totalbytes:
            ...
            self.file.read(self.skip_bytes) # TO 170702
    ...
    def _get_header(self):
        ...
        nlay = header1['nlay']
        if  nlay < 0:
            header2 = binaryread(self.file, self.header2_dtype, (1,), skip_bytes=self.skip_bytes) #TO, skip_bytes added
        else:
    ...
    def get_record(self, idx, full3D=False, verbose=False):
        ...
        # TO this is not correct see line 874, idx is an int
        #if np.isscalar(idx):
        #    idx = np.array([idx])
        ...
        imeth = header['imeth']  # TO

        t = header['text'] # TO
        s = 'Returning ' + str(t).strip() + ' as '

        nlay = abs(header['nlay']) # TO[0])
        nrow = header['nrow'] # TO [0]
        ncol = header['ncol'] # TO[0]
