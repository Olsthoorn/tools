...
def w_fr_x(X, xP=None, k=None):
    ...
    w = []
    for x in X:
        w.append(w_fr_one_x(x, xP=xP, k=k))        
...
def w_fr_zeta(Zeta, xP, k, N=100, W=2.5, endclip=1e-4):
    ...
    for zeta in Zeta.ravel():
        ...
        # zf = subdiv(0, z1, xP, N=N,  endclip=endclip)  
        ...
        zh = subdiv(zf[-1], zf[-1] + zeta.real, xP, N=N, endclip=endclip)
...
class SC_Section:
    ...
    # 1. Orde & configuratie
    # ----------------------------------------------------------
    def __init__(self, lengths, angles, ip1=0, ip2=1, W=2.5):
        """
    ...
    # ----------------------------------------------------------
    def solve_prevertices(self, N=25, W=2.5):
        ...
        res = least_squares(objective, u0,
                    xtol=1e-13, ftol=1e-13, gtol=1e-13, max_nfev=200, method='lm', args=[self.k, self.lengths, N, W])
        ...
        self.wP = w_fr_x(self.xP, xP=self.xP, k=self.k)
    ...
    # ----------------------------------------------------------
    def compute_full_map(self, Omega, N=100, endclip=1e-4):
        zeta0 = self.zeta0_fr_omega(Omega)
        zeta  = self.zeta_fr_zeta0(zeta0)
        w     = self.w_fr_zeta(zeta, N=100, endclip=endclip)
        z     = self.z_fr_w(w)
        return z
...
def test_sc_mapping(case=1):
    ...
    xsec = SC_Section(cases[case]['lengths'], cases[case]['angles'])
    w = xsec.w_fr_zeta(xsec.xP)
    
    fig, ax = plt.subplots()
    ax.plot(w.real, w.imag, '.-')

    ax.set_title(cases[case]['title'])
...
def some_shape(case_title='Ditch',
    ...
    xsec = SC_Section(lengths, angles, ip1=ip1, ip2=ip2, W=W)
    ...
    w     = xsec.w_fr_zeta(zeta, N=N, endclip=endclip)
    Z     = xsec.z_fr_w(w)
...
if __name__ == '__main__':
    ...
    if True:
        test_sc_mapping(case=0)
        test_sc_mapping(case=1)
        test_sc_mapping(case=2)
        test_sc_mapping(case=3)
        test_sc_mapping(case=4)
