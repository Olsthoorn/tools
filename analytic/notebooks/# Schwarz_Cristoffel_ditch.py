...
k = [0.5, -0.5, 0.5, 0.5]
b, c, e = 2, 0.5, 1.5
B, C, D, E = b + 0j, b - c * 1j, 0 - c * 1j, 0 - e * 1j
...
def sc_along_real_ax(x, xP, k):
    ...
    xP_ = np.hstack((0, xP))
    for x1, x2 in zip(xP_[:-1], xP_[1:]):        
        a = x1
    
        if x <= a:
            break
        
        b = min(x, x2) # x may be < than b
...
p = (B - C) / (xP_new[0] - xP_new[1])
q = (xP_new[0] * C - xP_new[1] * B ) / (xP_new[0] - xP_new[1])
...
print(f"{B:.3f}, {C:.3f}, {D:.3f}, {E:.3f}")
print(f"{bb:.3f}, {cc:.3f}, {dd:.3f}, {ee:.3f}")
