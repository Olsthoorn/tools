...
def hemk99numerically(t=None, r=None, z=None, rw=None, rc=None, topclosed=True, botclosed=True, **kw):
    ...
    Setup an axial finite difference model for wells that may have multiple screens but uniform head inside the well.
    
    Well storage is generated by taking a large value for rc (the radius of the casing in which the water table varies).
    
    The idea is to generate a simulation that is immediately comparible with the analytic multilayer solution by Hemerk1999
    
    The distances given will be changed to make sure the first value equals 0, the second equals rw (well radius)
    and the rest are as given, but only for values > rw. Hence, the distance array will be:
    
    np.array([0, rw, r[r>rw]])
    
    The well is implemented by using a very high value for kv while using kh=0 where
    unscreened parts and the same high kv value for screened parts.
    
    Parameters
    ----------
    t: simulation times
        times
    z: None, sequence or array
       elevation of layer planes (tops and bottoms are in one array from top to bottom)
    rw: float
