...
# %%
if True:
    methods = ['dehoog'] # ['cohen', 'talbot', 'dehoog', 'stehfest']
    method = 'dehoog'
    ...
    for r in rs[1:2]:
        ...
        if use_mpm:
            ...
            for method in methods:
                ...
                for it, t in enumerate(ts):
                    print(f"t = {t}")
                    fi[it] = mpm.invertlaplace(FP, t, method=method, tol=tol, degree=degree)
                    qi[it] = mpm.invertlaplace(QP, t, method=method, tol=tol, degree=degree)
                ax1.plot(ts, fi, color=clr, label=f"{method} r={r}, R={R}")
                ax2.plot(ts, qi, color=clr, label=f"{method} r={r}, R={R}")
            ...
            for method in methods:
                ...
                F_ = invertor.calc_time_domain_solution(fpF, ts, kwargs)
                Q_ = invertor.calc_time_domain_solution(fpQ, ts, kwargs)

                ax1.plot(ts, F_, 'o-', color=clr, mfc='none', label=f'{method}, r={r}')
                ax2.plot(ts, Q_, 'o-', color=clr, mfc='none', label=f'{method}, r={r}')
