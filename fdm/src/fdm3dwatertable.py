# [markdown]
# # Phreatic 3D transient groundwater flow in a water table aquifer
#
# This version of fdm3 takes into account variabl thickness of a
# water-table aquifer. So D and kD are not longer constant.
#
# Just run this file.
#
# TO 2023-03-20
...
def inside(xlim, zlim, X, Z):
    return np.logical_and(X > xlim.min(),
...
def fillLayer(gr, shapes=None, field=None, dtype=float, fillValue=0):
    ...
    startvalue
    """
    if field not in shapes.columns:
...
def setBound(gr=None, A=None, recarr=None):
    ...
    for rec in recarr:
        ...
        if z1 > z2:
            z1, z2 = z2, z1        
        Idx = inside(np.array([x1, x2]), np.array([z1, z2]), gr.XM, gr.ZM)
        if np.sum(Idx) == 0:
...
def fdm3wtt(gr=None, t=None, kxyz=None, Ss=None, Sy=None,
    ...
    def R(x):
        return x.ravel()  # generate anonymous function R(x) as shorthand for x.ravel()
    ...
    for it, dt in enumerate(np.diff(t)):
        ...
        if steady and np.all(fxhd == np.False_):
            raise ValueError(
        ...
        for iouter in range(Nouter):
            ...
            if it == Nouter:
                if verbose:
                    print(f"Outer convergence failed at t[{it}] = {t:.2f} d, err={err}")
...
if __name__ == '__main__':
    ...
    # --- define axially symmetric 2-layer grid
    ...
    # --- flow (extraction negative, infiltration positive)
    Q = 60 * 24
