# -*- coding: utf-8 -*-
# This module, named mfgrid, has Grid class as its important item.
# Grid is instantiated to know the classic bock-shaped 3D Modflow Grid,
# after which any grid info can be supplied by its methods and properties.
# Grid has been developed from 2008 ans was used with every fdm simulation.
#
# For Modflow 6 see mfgridVDS.py
...
# module. Grid has been developed from 2008.
#
# For Modflow 6 see mfgridVDS.py
...
class Grid:
    ...
    def const(self, u, dtype=float, axial=False, lay=True, cbd=False):
        ...
        if isinstance(u, (np.ndarray, list, tuple)):
            u = np.array(u).ravel()[:, np.newaxis, np.newaxis]
            nz = len(u)
        elif lay is True:
            nz = self._nlay
        elif cbd is True:
