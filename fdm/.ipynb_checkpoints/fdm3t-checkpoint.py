...
def fdm3t(gr=None, t=None, kxyz=None, c=None, Ss=None, GHB=None,
    ...
    sure at least one cell is fixed head, i.e. one IDOMAIN value < 0.
    ...
    kxyz: 3-tuple of ndarray, shape: (Nz, Ny, Nx), [L/T]
    ...
    FQ : Fixed-flow boundaries dict{ it: recarray, ...}
        recarray has dtype
            dtype = np.dtype([('I', int), ('Q', float)])
    FH:  Fixed-head or perscribed heads dict {it: recarray, ....}
        recarray had dtype:
            dtype = np.dtype([('I', int), ('h', float)])
    ...
    if np.all(Ss == 0.):
        if all(fxhd == False) and GHB is None:
            raise ValueError('If all Ss == 0, at least one fxhd cell is required, i.e. one IDOMAIN value must be -1 or GHB must be used.')
        else:
    ...
    Qghb= np.zeros((Nt, gr.nz, gr.ny, gr.nx)

    # reshape input arrays to vectors for use in system equation
    HI = R(HI);  Cs = R(Cs)
    
    fixedQ = {0, np.zeros(1, dtype=dtypeQ   if FQ  is None else {0: R(FQ)} if isinstance(FQ, np.ndarray) else FQ
    fixedH = {0, np.zeros(1, dtype=dtypeH   if FH  is None else {0, R(FH)} if isinstance(FH, np.ndarray) else FH
    GHB    = {0, np.zeros(1, dype=dtypeGHN} if GHB is None else GHB

    assert fixedQ[0].dtype = dtypeQ, 'fixed Q dict must have dtype {}'.format(dtypeQ)
    assert fixedH[0].dtype = dtypeH, 'fixed h dict must have dtype {}'.format(dtypeH)
    assert GHB.dtype == dtypeGHB , 'GHB must have dtype {}'.format(dtypeGHB)
    ...
    for idt, dt in enumerate(np.diff(t)):
        ...
        if idt in fixedH:
            ...
            fxhd[fixedH[idt]['I'] = 1
            FH.ravel()[:] = 0.
            FH.ravel()[fixedH[idt]['I'] = fixedH[idt]['h']
        if idt in fixedQ:
            FQ.ravel()[:] = 0.
            FQ.ravel()[fixedQ[idt]['I']  = fixedQ[idt]['Q']
        if idt in GHB:
            Cghb.ravel()[:] = 0.
            Hghb.ravel()[:] = 0.
            Hghb.ravel()[GHB[idt]['I'] = GHB[idt]['h']
            Cghb.ravel()[GHB[idt]['I'] = GHB[idt]['C']
            
        it = idt+1
          
        Phi[it][fxhd] = FH.ravel()[fxhd]      
...
class Fdm3t:
    ...
    def __init__(self, gr=None, t=None, kxyz=None, Ss=None, FQ=None, HI=None,
        ...
        self.out = fdm3t(gr=self.gr, t=self.t,
                         ...
                         GHB=self.GHB,
                         IDOMAIN=IDOMAIN, epsilon=epsilon)
...
def testPars(kw):
    ...
    Kr  = gr.const(kw['kr'])
    Kz  = gr.const(kw['kz'])
    return gr, kD, S, Kr, Kz, Ss, Q, HI, FQ, IDOMAIN

def steady(kw):
    """Return De Glee output in a two-layer axially symmetric layer.
    
    Steady state is computed be setting all Ss = 0. and setting IDOMAIN < 0 for at least
    ...
    """
    gr, kD, S, Kr, Kz, Ss, Q, HI, FQ, IDOMAIN = testPars(kw)
       
    IDOMAIN[0] = -1 # Top layer fixed.
    Ss.ravel()[:] = 0
    kw['t'] = kw['t'][:2] # only need a single timestep, but more is ok.
     
    c = gr.const(kw['c'])
        
    out = fdm3t(gr=gr, t=kw['t'], kxyz=(Kr, Kr, Kz), Ss=Ss, FQ=FQ, HI=HI, IDOMAIN=IDOMAIN, c=c)
...
def theis1(kw):
    ...
    one IDOMAIN value must be -1 (fixed head). In this test function this is
    ...
    out = fdm3t(gr=gr, t=kw['t'], kxyz=(Kr, Kr, Kz), Ss=Ss, FQ=FQ, HI=HI, IDOMAIN=IDOMAIN, epsilon=epsilon)
...
def theis(kw):
    ...
    out = fdm3t(gr=gr, t=t, kxyz=(Kr, Kr, Kz), Ss=Ss, FQ=FQ, HI=HI, IDOMAIN=IDOMAIN, epsilon=1.0)
...
def hantush1L(kw):
    ...
    for rho in kw['rhos']:
        ...
        out = fdm3t(gr=gr, t=t, kxyz=(Kr, Kr, Kz), Ss=Ss, FQ=FQ, HI=HI, IDOMAIN=IDOMAIN, c=c, GHB=GHB, epsilon=1.0)
