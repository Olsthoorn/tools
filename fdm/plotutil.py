...
def cell_value_points(pts, xGr, yGr, zGr, vdata, mid=False):
    ...
    Intersect a list of polyline vertices with a rectilinear MODFLOW
    grid. Vertices at the intersection of the polyline with the grid
    cell edges are computed. If mid==False the cell values are computed
    at thise poitns. If mid is true, the cell values at successive mid points
    are returned to make sure each values falls in one cell
    are returned.
