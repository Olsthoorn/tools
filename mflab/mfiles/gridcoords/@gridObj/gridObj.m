...
%         nm, nc   % same as ym yx but in wgs latitude  degrees                
    ...
    methods
        ...
        function Iwtbl = IwaterTable(o,H)
            %GRIDOBJ.IwaterTable finds the highest index of every grid column not being
            %a NaN (which normally indicates dry cells). So first wet cell
            %
            % TO 151124
...
%                 end
        ...
        function showPoint(o,L,R,C,varargin)
            %gridObj/showPoint -- shows a point on a cross section
            % Useful when MODFLOW fails to converge and one wants to
            % identify the point in the network where this happens.
            %
            % USAGE gr.showPoint(Layer,Row,Col,[plotoptions])
            %
            % layer row col of the cel are obtained from the LST file where the
            % point of max error is given.
            % plotOptions are passed as given to the regular plot function
            % if omitted plotOptions= 'yo','markerSize',10
            %
            % TO 160412
            
            if isempty(varargin)
                varargin={'yo','markerSize',10};
            end
            plot(o.xm(C),o.ZM(R,C,L),varargin{:})
        end
