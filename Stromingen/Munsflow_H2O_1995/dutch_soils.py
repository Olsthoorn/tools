...
class Soil():
    ...
    @staticmethod
    def alpha_wet(psi, psi1=10.0, psi2=25.0):
        """Return suffocation factor for wet conditions."""
        if psi > psi2:
            return 1.0
        if psi1 < psi < psi2:
            return (psi - psi1) / (psi2 - psi1)
        else:
            return 0.

    def feddes_alpha(self, theta, pf_h1=1, pf_h2=1.8, pf_h3=3.25, pf_h4=4.2):
        """Return evaportranpiration reduction factor acc. to Feddes.
        
        Parameters
        ----------
        theta: float, ndarray
            moisture content (will be converted to pF = np.log10(psi))
        
        pF: float
            log(-h [cm]) or log(psi [cm]) (< 0)
            pressure heads are all negative, suction heads are all postive [cm]negative).

        Waarden volgens Feddes
        1 < psi1 <   10 cm (0.0 < pf1 < 1.0) geom. avg pF = 0.5
        25 < psi2 <  200 cm (1.4 < pF2 < 2.3) geom. avg pF = 1.85
        400 < psi3 < 8000 cm (2.6 < pF3 < 3.9) geom. avg pF = 3.25
        """
        pF = np.log10(self.psi_fr_theta(theta))
        psi, psi1, psi2, psi3, psi4 = 10 ** pF, 10 ** pf_h1, 10 ** pf_h2, 10 ** pf_h3, 10 ** pf_h4
        if psi < psi1:
            return 0.0
        elif psi1 <= psi < psi2:
            return (psi - psi1) / (psi2 - psi1)
    ...
    def smooth_alpha(self, theta, s=0.2, pF50=3.6, psi2=25):
        ...
        Choose:
            3.5 < pF50 < 3.8
            0.15 <  s  < 0.30 # Controls steepness
        """
        psi = self.psi_fr_theta(theta)
        pF, pF2 = np.log10(psi), np.log(psi2)
        if pF > 4.2:
            return 0.
        if pF2 < pF <= 4.2:
            return 1 / (1 + + np.exp((pF - pF50) / s))
        else:
            return self.alpha_wet(psi, psi2=psi2)
    ...
    def mualem_alpha(self, theta, beta=1.0, psi2=25):
        ...
        if theta < th_wp:
            return 0.
        if th_wp < theta < th_fc:
            return (self.K(theta) / self.K(th_fc)) ** beta
        if th_suff < theta <= th_fc:            
            return 1.0
        else:
            return self.alpha_wet(theta, psi2=psi2)
            
# %%
dirs = Dirs('~/GRWMODELS/python/tools/Stromingen/Munsflow_H2O_1995')
